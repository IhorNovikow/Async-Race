...
export function loadWiner(name: string, color: string, id: number, time: number, wins: number) {
    const ol = <HTMLElement>document.querySelector('.winner-ol');
    const li = CreateElement('li', 'inf-winner');
    li.id = String(id);
    const modelCar = CreateElement('div', 'model-car');
    modelCar.innerHTML = `
                <svg xmlns="http://www.w3.org/2000/svg" version="1.0" viewBox="0 0 1280.000000 640.000000" preserveAspectRatio="xMidYMid meet" fill = "${color}"class = "model-card">
                ...
                `;
    const modelName = CreateElement('div', 'model-name');
    modelName.innerHTML = name;
    const modelTime = CreateElement('div', 'model-best-time');
    modelTime.innerHTML = String(time);

    const countWins = CreateElement('div', 'head-table-title');
    countWins.classList.add('CountWiner');
    countWins.innerHTML = String(wins);

    li.appendChild(modelCar);
    li.appendChild(modelName);
    li.appendChild(countWins);
    li.appendChild(modelTime);
    ol.appendChild(li);
}
