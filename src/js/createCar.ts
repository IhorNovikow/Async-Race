...
export function createCar(name: string, color: string, id: string) {
    const mainContent = <HTMLElement>document.querySelector('.main-content');
    const mainContentWrapper = CreateElement('div', 'main-content-wrapper');
    mainContentWrapper.id = id;
    const c = CreateElement('div', 'c');
    const mainContentTop = CreateElement('div', 'main-content-top');
    const mainContentButton = CreateElement('div', 'main-content-button');
    const mainSelect = CreateElement('button', 'main-select');
    mainSelect.innerHTML = 'Сhange';
    mainSelect.setAttribute('id', 'change');
    const mainRemove = CreateElement('button', 'main-remove');
    mainRemove.innerHTML = 'Remove';
    const mainContentName = CreateElement('div', 'main-content-name');
    mainContentName.innerHTML = name;
    const mainContentSubbutton = CreateElement('div', 'main-content-subbutton');
    const mainA = CreateElement('button', 'main-A');
    mainA.innerHTML = 'A';
    const mainB = CreateElement('button', 'main-B');
    mainB.innerHTML = 'B';
    mainB.setAttribute('disabled', 'true');
    const imgCar = CreateElement('div', 'img-car');
    imgCar.innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" version="1.0" viewBox="0 0 1280.000000 640.000000" preserveAspectRatio="xMidYMid meet" fill = "${color}" class = "svg-card">
    ...
    mainContentButton.appendChild(mainSelect);
    mainContentButton.appendChild(mainRemove);
    mainContentTop.appendChild(mainContentButton);
    mainContentTop.appendChild(mainContentName);
    c.appendChild(mainContentTop);
    mainContentSubbutton.appendChild(mainA);
    mainContentSubbutton.appendChild(mainB);
    c.appendChild(mainContentSubbutton);
    mainContentWrapper.appendChild(c);
    mainContentWrapper.appendChild(imgCar);
    mainContentWrapper.appendChild(flag);
